...
function record(value: unknown, label: string): RawRecord {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error(`Invalid browser provider ${label}.`);
  return value as RawRecord;
}

function stringValue(value: unknown, label: string): string {
  if (typeof value !== 'string' || !value.trim()) throw new Error(`Invalid browser provider ${label}.`);
  return value.trim();
}

function locator(value: unknown, label: string): ShadowLocator {
  const source = record(value, label);
  if (!Array.isArray(source.segments) || source.segments.length === 0 || source.segments.some((item) => typeof item !== 'string' || !item.trim())) {
...
export function validateBrowserProviderProfile(value: unknown): BrowserProviderProfile {
  ...
  if (Object.prototype.hasOwnProperty.call(source, 'script') || Object.prototype.hasOwnProperty.call(source, 'scriptSource')) {
  ...
  if (!Array.isArray(match.origins) || match.origins.length === 0) throw new Error('Browser provider match origins are required.');
  ...
  if (inputMode !== 'input' && inputMode !== 'textarea' && inputMode !== 'contenteditable') throw new Error('Invalid browser provider composer input mode.');
  const submit = record(source.submit, 'submit');
  if (submit.action !== 'click' && submit.action !== 'enter') throw new Error('Invalid browser provider submit action.');
  if (submit.action === 'click' && !submit.locator) throw new Error('Browser provider click submit locator is required.');
  const assistant = record(source.assistantMessages, 'assistantMessages');
  const completion = record(source.completion, 'completion');
  if (typeof completion.stabilityMs !== 'number' || !Number.isFinite(completion.stabilityMs) || completion.stabilityMs < 250) throw new Error('Invalid browser provider completion stability.');
  const learned = record(source.learned, 'learned');
  const confidence = record(learned.confidence ?? {}, 'confidence');
  for (const value of Object.values(confidence)) if (value !== 'high' && value !== 'medium' && value !== 'low') throw new Error('Invalid browser provider confidence.');
  const result: BrowserProviderProfile = {
    ...
    assistantMessages: { locator: locator(assistant.locator, 'assistantMessages'), textLocator: optionalLocator(assistant.textLocator, 'assistantMessages text') },
    completion: { generatingLocator: optionalLocator(completion.generatingLocator, 'generating'), idleLocator: optionalLocator(completion.idleLocator, 'idle'), stabilityMs: completion.stabilityMs },
    learned: {
      sourceOrigin: origin(learned.sourceOrigin, 'source'),
      createdAt: stringValue(learned.createdAt ?? new Date().toISOString(), 'createdAt'),
      updatedAt: stringValue(learned.updatedAt ?? learned.createdAt ?? new Date().toISOString(), 'updatedAt'),
  ...
  if (source.newConversation !== undefined) {
    ...
    if (newConversation.action !== 'click' && newConversation.action !== 'navigate') throw new Error('Invalid browser provider new conversation action.');
    if (newConversation.action === 'click' && !newConversation.locator) throw new Error('Browser provider new conversation locator is required.');
    if (newConversation.action === 'navigate' && typeof newConversation.url !== 'string') throw new Error('Browser provider navigation URL is required.');
    result.newConversation = { action: newConversation.action, locator: optionalLocator(newConversation.locator, 'newConversation'), url: typeof newConversation.url === 'string' ? newConversation.url : undefined };
  }
  return result;
}

export function parseBrowserProviderProfiles(value: unknown): Record<string, BrowserProviderProfile> {
  ...
  for (const [id, raw] of Object.entries(source)) {
    ...
    const normalized = validateBrowserProviderProfile({
      ...
      submit: { ...submit, locator: submit.locator },
