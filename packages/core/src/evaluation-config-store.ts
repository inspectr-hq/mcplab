...
/** The canonical filename policy used by the app API and MCP creation tool. */
...
/**
 * Creates a normalized MCPLab evaluation config in the configured evals root.
 * Both the app API and MCP server call this function to keep persistence rules identical.
 */
export function createEvaluationConfigFile(params: {
  ...
  while (existsSync(target)) {
    target = join(evalsDir, `${fileName}-${suffix}.yaml`);
    suffix += 1;
  }
