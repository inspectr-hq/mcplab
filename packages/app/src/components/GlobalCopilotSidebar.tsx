...
function stored(message: Message): GlobalCopilotMessage | null {
  ...
  const externalTool = message.role === 'assistant'
    ? (message as Message & { toolCalls?: Array<{ function: { name: string; arguments: string } }> }).toolCalls?.find((call) => call.function.name !== 'navigate_to_view' && !call.function.name.startsWith('mcplab__'))
    : undefined;
  if (externalTool) {
    try {
...
export function GlobalCopilotSidebar() {
  ...
  return <aside className="hidden w-[360px] shrink-0 border-l bg-card lg:flex lg:flex-col" aria-label="Global copilot">
    ...
    <ScrollArea className="min-h-0 flex-1"><div className="space-y-3 p-3">{messages.length === 0 && <p className="text-sm text-muted-foreground">Ask about results, test cases, or MCPLab.</p>}{messages.map((message) => <div key={message.id} className="space-y-2"><AssistantMessageRow message={{ id: message.id, role: message.role, text: message.content, createdAt: message.createdAt }} />{message.action?.kind === 'navigate_to_view' && message.action.status === 'pending' && <div className="rounded-md border border-amber-400/40 bg-amber-50 p-2 text-sm"><p>{message.action.reason || `Open ${message.action.path}?`}</p><div className="mt-2 flex gap-2"><Button size="sm" onClick={() => void confirmNavigation(message, true)}>Open view</Button><Button size="sm" variant="outline" onClick={() => void confirmNavigation(message, false)}>Not now</Button></div></div>}{message.action?.kind === 'external_mcp_tool' && message.action.status === 'pending' && <div className="rounded-md border border-amber-400/40 bg-amber-50 p-2 text-sm"><p>Run {message.action.serverName}/{message.action.toolName}?</p><div className="mt-2 flex gap-2"><Button size="sm" onClick={() => void confirmExternalTool(message, true)}>Run tool</Button><Button size="sm" variant="outline" onClick={() => void confirmExternalTool(message, false)}>Not now</Button></div></div>}{message.action?.kind === 'navigate_to_view' && message.action.status === 'approved' && <p className="text-xs text-emerald-700">Opened {message.action.path}</p>}{message.action?.status === 'denied' && <p className="text-xs text-muted-foreground">Action declined</p>}{message.action?.status === 'error' && <p className="text-xs text-destructive">Action failed</p>}</div>)}{loading && <AssistantTypingIndicator />}</div></ScrollArea>
