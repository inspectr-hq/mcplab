...
export function GlobalCopilotSidebar() {
  ...
  return <aside className="hidden w-[360px] shrink-0 border-l bg-card lg:flex lg:flex-col" aria-label="Global copilot">
    ...
    <ScrollArea className="min-h-0 flex-1"><div className="space-y-3 p-3">{messages.length === 0 && <p className="text-sm text-muted-foreground">Ask about results, test cases, or MCPLab.</p>}{messages.map((message) => <AssistantMessageRow key={message.id} message={{ id: message.id, role: message.role, text: message.content, createdAt: message.createdAt }} />)}{loading && <AssistantTypingIndicator />}</div></ScrollArea>
