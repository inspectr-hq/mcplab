  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-0">
        ...
        {groupBy === "severity" ? (
          <div className="space-y-4">
          {ALL_SEVERITIES.filter((severity) => reportSeveritySet.has(severity)).map((severity) => {
            const entries = findingsBySeverity[severity].filter((entry) => matchesToolFilter(entry.toolLabel));
            ...
            return (
              <Card key={severity}>
                ...
                {!isCollapsed && <CardContent className="space-y-2">
                    ...
                    <div key={entry.key} className="rounded border p-2 text-xs">
                      <div className="mb-1 flex items-center justify-between gap-2">
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          <span className="min-w-0 font-bold leading-tight">{entry.issue.title}</span>
                        </div>
                        <Badge variant="outline" className={`shrink-0 text-[10px] ${severityBadgeClass(entry.issue.severity)}`}>{entry.issue.severity}</Badge>
                      </div>
                      <p><span className="font-bold">Finding:</span> {entry.issue.detail}</p>
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                        <Badge variant="secondary" className="font-normal">{entry.sourceLabel}</Badge>
                        <Button
                          type="button"
                          variant="ghost"
                          size="sm"
                      ...
                      {entry.issue.suggestion && <SuggestionCallout text={entry.issue.suggestion} />}
                    </div>
                  ))}
                </CardContent>}
              </Card>
