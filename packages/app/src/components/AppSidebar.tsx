...
export function AppSidebar(props: AppSidebarProps = { version: null }) {
  ...
  return (
    <Sidebar collapsible="icon">
      <SidebarHeader
        className={`border-b border-sidebar-border py-3 ${collapsed ? 'px-2' : 'px-4'}`}
      >
        <div
          className={`flex items-center ${
            collapsed ? 'justify-center overflow-visible' : 'gap-2.5 overflow-hidden'
          }`}
        >
      ...
      <SidebarFooter className={collapsed ? 'px-2 py-2' : 'px-3 py-3'}>
        ...
        <div
          className={`flex ${
            collapsed ? 'justify-center' : 'items-center justify-center gap-2'
          } w-full`}
        >
          <a
