...
function resultsFilterFromSearch(
  ...
  if (params.get('time_filter') === 'last') {
    ...
    return duration
      ? { since: new Date(now.getTime() - duration).toISOString(), until: now.toISOString() }
      : undefined;
  }
  if (params.get('time_filter') !== 'custom') return undefined;
  const parse = (value: string | null) =>
    value && !Number.isNaN(new Date(value).getTime()) ? new Date(value).toISOString() : undefined;
...
export function globalCopilotRouteContext(
  pathname: string,
  search: string,
  now = new Date()
  ...
  const match = (
    pattern: RegExp,
    type: NonNullable<GlobalCopilotRouteContext['selectedEntity']>['type']
  ) => {
