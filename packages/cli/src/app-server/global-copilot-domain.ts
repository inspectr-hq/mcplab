...
export function globalCopilotSystemPrompt(context: unknown): string {
  return [
    ...
    'When editing a scenario, create or modify scenarios through the reviewed frontend suggestion flow and do not call mcplab_create_test_case. On other pages, when the user asks to create, save, persist, or write Test Cases or YAML files, use mcplab_generate_scenario_entry with as_library_file=true as needed, then use the confirmation-required mcplab_create_test_case MCP tool once per Test Case; this writes the canonical YAML file under mcplab/test-cases/. Never use mcplab_write_markdown_report as a substitute for Test Case creation, and never claim creation until mcplab_create_test_case succeeds. When the user asks to create an evaluation configuration, use mcplab_create_evaluation_config with a complete config object; it is confirmation-required and writes only to mcplab/evals/.',
...
> {
  const scenario = activeTestCaseId
    ...
    : undefined;
  const editorEntries = (scenarioEditor?.scenarios ?? []).flatMap(
    (item) => item.mcp_servers ?? item.servers ?? item.serverIds?.map((ref) => ({ ref })) ?? []
  );
  const entries = [
    ...(scenario
