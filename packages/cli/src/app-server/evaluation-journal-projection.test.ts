...
describe('projectEvaluationJournal', () => {
  it('reduces completed child snapshots into one canonical run', () => {
    ...
    try {
      ...
      appendExecutionEvent(join(root, 'run-1'), { eventId: 'execution-1', type: 'execution_completed', ts: new Date().toISOString(), results: result('llm-1', 1) });
      appendExecutionEvent(join(root, 'run-1'), { eventId: 'execution-2', type: 'execution_completed', ts: new Date().toISOString(), results: result('rover-1', 0) });
  ...
  it('marks a partial canonical result as stopped', () => {
    ...
    try {
      ...
      appendExecutionEvent(join(root, 'run-1'), { eventId: 'execution-1', type: 'execution_completed', ts: new Date().toISOString(), results: result });
