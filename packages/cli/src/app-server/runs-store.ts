...
export function toTraceUiEvents(events: TraceEvent[]): TraceUiEvent[] {
  ...
  for (const event of events) {
    if (event.type === 'scenario_started') {
      ...
      normalized.push({ type: 'scenario_started', scenario_id: event.scenario_id, ts: event.ts });
      continue;
    }
    ...
    if (event.type === 'llm_response') {
      normalized.push({ type: 'llm_response', raw_or_summary: event.raw_or_summary, ts: event.ts });
      continue;
    }
