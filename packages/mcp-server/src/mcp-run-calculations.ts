...
export function computeMetricSummary(scenarios: ResultsJson['scenarios']): MetricSummary {
  ...
  const evaluatedRuns = runs.filter((r) => (r as { evaluation_status?: string }).evaluation_status !== 'skipped').length;
  const skippedRuns = totalRuns - evaluatedRuns;
  const passedRuns = runs.filter((r) => {
    const status = (r as { evaluation_status?: string }).evaluation_status;
    return status !== undefined ? status === 'passed' : r.pass;
  }).length;
  const failedRuns = runs.filter((r) => {
    const status = (r as { evaluation_status?: string }).evaluation_status;
    return status !== undefined ? status === 'failed' : !r.pass;
  }).length;
