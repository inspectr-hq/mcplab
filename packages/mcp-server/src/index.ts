...
async function main(): Promise<void> {
  ...
  const httpServer = createServer(async (req, res) => {
    try {
      await handleHttpRequest(req, res, sessions);
    } catch (error) {
      console.error('[mcplab-mcp-server] request error:', error);
      if (!res.headersSent) {
        sendJson(res, 500, {
          jsonrpc: '2.0',
  ...
  httpServer.listen(MCP_PORT, MCP_HOST, () => {
    console.error(
      `[mcplab-mcp-server] Streamable HTTP listening on http://${MCP_HOST}:${MCP_PORT}${MCP_PATH}`
    );
  });
  ...
  const shutdown = async () => {
    for (const [sessionId, runtime] of sessions) {
      try {
        ...
        console.error(`[mcplab-mcp-server] failed to close session ${sessionId}:`, error);
      }
    }
    sessions.clear();
