# MCPLab Documentation Restructure Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Restructure the MCPLab website docs from 6 flat pages to a 15-page two-track layout (CLI + App) with Getting Started and Reference sections, documenting all currently undocumented features.

**Architecture:** All page content lives in `packages/website/src/data/docs.ts` as structured `DocPage` objects. The `.astro` page files are thin shells (~14 lines each) that call `getDocPageBySlug()` and pass sections to `<DocContent>`. Updating `docs.ts` is the main work; creating page files is mechanical. Astro's built-in `redirects` config handles legacy URL preservation.

**Tech Stack:** Astro SSG, TypeScript, `DocSection` / `DocPage` types in `docs.ts`, `DocContent.astro` component, `astro.config.mjs` for redirects.
...
```
packages/website/
├── astro.config.mjs                                        MODIFY  (add redirects)
├── src/
│   ├── data/
│   │   └── docs.ts                                         MODIFY  (rewrite: 15 pages + nav)
│   └── pages/
│       └── docs/
│           ├── index.astro                                 MODIFY  (update grid)
│           ├── installation.astro                          MODIFY  (update section label)
│           ├── quick-start.astro                           MODIFY  (update section label)
│           ├── configuration.astro                         DELETE  (redirect replaces it)
│           ├── usage.astro                                 DELETE  (redirect replaces it)
│           ├── app-mode.astro                              DELETE  (redirect replaces it)
│           ├── cli/
│           │   ├── running-evaluations.astro               CREATE
│           │   ├── configuration.astro                     CREATE
│           │   ├── reports-output.astro                    CREATE
│           │   └── ci-cd.astro                             CREATE
│           ├── app/
│           │   ├── getting-started.astro                   CREATE
│           │   ├── running-evaluations.astro               CREATE
│           │   ├── reading-results.astro                   CREATE
│           │   ├── ai-assistants.astro                     CREATE
│           │   ├── tool-analysis.astro                     CREATE
│           │   └── library.astro                           CREATE
│           └── reference/
│               ├── configuration.astro                     CREATE
│               └── environment-variables.astro             CREATE
```

**Key constraint:** `DocSection` supports `paragraphs: string[]`, `bullets: string[]`, `codeBlocks: { title?, language?, code }[]`. No tables — use grouped sections with bullets and code blocks for reference content.

---

## Task 1: Rewrite `docs.ts` — Navigation + Getting Started

**Files:**
- Modify: `packages/website/src/data/docs.ts`

**Context:** `docs.ts` currently has 6 pages in a flat `pageIndex` array. The `docsNavSections` export drives the sidebar. The `getDocPageBySlug` function does a `.find()` by `slug` field. All `.astro` page shells call `getDocPageBySlug('slug')!` to load their content.

The rewrite keeps `DocPage`, `DocSection`, `DocNavItem` type definitions unchanged. It replaces the `pageIndex` array and `docsNavSections` export.

- [ ] **Step 1: Replace `docs.ts` with the full new version**

Write the following complete file at `packages/website/src/data/docs.ts`:
...
scenarios:
  - id: basic-test
    ...
    },
    {
      id: 'run-it',
      title: 'Run It',
      paragraphs: ['Point MCPLab at your config and it will run all scenarios and generate a report.'],
      codeBlocks: [
    ...
    },
    {
      id: 'next-steps',
      title: 'Next Steps',
      paragraphs: ['Follow the CLI track to learn about all run options, configuration patterns, and CI integration. Follow the App track to run evaluations from a UI with AI assistants built in.'],
    },
  ],
};
...
const cliRunning: DocPage = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: ['Run a single scenario by its ID using -s. Pass the flag multiple times to run several.'],
      codeBlocks: [
        { title: 'single scenario', language: 'bash', code: 'mcplab run -c eval.yaml -s basic-test' },
        { title: 'multiple scenarios', language: 'bash', code: 'mcplab run -c eval.yaml -s test-one -s test-two' },
      ],
    },
    {
      id: 'select-agents',
      ...
      codeBlocks: [
        { title: 'specific agents', language: 'bash', code: 'mcplab run -c eval.yaml --agents claude,gpt4o' },
        { title: 'all agents (config + library)', language: 'bash', code: 'mcplab run -c eval.yaml --agents-all' },
      ],
    },
    {
      id: 'variance-runs',
    ...
    },
    {
      ...
      codeBlocks: [
        { title: 'interactive', language: 'bash', code: 'mcplab run --interactive' },
      ],
    },
    {
      id: 'annotate-runs',
      ...
      codeBlocks: [
        { title: 'annotated run', language: 'bash', code: 'mcplab run -c eval.yaml --run-note "after refactor"' },
        { title: 'custom output dir', language: 'bash', code: 'mcplab run -c eval.yaml --runs-dir ./my-runs' },
      ],
    },
    {
      id: 'exit-codes',
      title: 'Exit Codes',
      paragraphs: ['mcplab run exits 0 when all scenarios pass and non-zero when any scenario fails. Use this in CI to fail a pipeline on a regression.'],
    },
  ],
};
...
const cliConfiguration: DocPage = {
  ...
  keywords: ['config', 'yaml', 'servers', 'agents', 'scenarios', 'assertions', 'bearer token', 'refs'],
  ...
  sections: [
    {
      ...
      paragraphs: ['An eval file has three required top-level keys: servers, agents, and scenarios.'],
      codeBlocks: [
        {
...
Run: `cd packages/website && npx tsc --noEmit`
Expected: no errors

- [ ] **Step 3: Commit**

```bash
git add packages/website/src/data/docs.ts
git commit -m "feat(docs): rewrite docs.ts with 15-page two-track structure and full content"
```

---

## Task 2: Create CLI Track Page Files

**Files:**
- Create: `packages/website/src/pages/docs/cli/running-evaluations.astro`
- Create: `packages/website/src/pages/docs/cli/configuration.astro`
- Create: `packages/website/src/pages/docs/cli/reports-output.astro`
- Create: `packages/website/src/pages/docs/cli/ci-cd.astro`

**Context:** Page files are ~14-line shells. They call `getDocPageBySlug('slug')!` using the slug defined in `docs.ts` and pass sections to `<DocContent>`. The section label above the title identifies which track the page belongs to.

- [ ] **Step 1: Create all four CLI page files**

`packages/website/src/pages/docs/cli/running-evaluations.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
import { getDocPageBySlug } from '@/data/docs';

const page = getDocPageBySlug('cli-running-evaluations')!;
---

<DocsLayout title={page.seoTitle} description={page.description} currentPath={page.href}>
  <p class="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">CLI</p>
  <h1 class="mt-3 font-display text-4xl font-bold tracking-tight">{page.label}</h1>
  <p class="mt-4 text-lg text-muted-foreground">{page.description}</p>
  <DocContent sections={page.sections} />
</DocsLayout>
```

`packages/website/src/pages/docs/cli/configuration.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('cli-configuration')!;
---
...
`packages/website/src/pages/docs/cli/reports-output.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('cli-reports-output')!;
---
...
`packages/website/src/pages/docs/cli/ci-cd.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('cli-ci-cd')!;
---
...
Run: `cd packages/website && npm run dev`

Check:
- http://localhost:8558/docs/cli/running-evaluations — shows 7 sections
- http://localhost:8558/docs/cli/configuration — shows 7 sections
- http://localhost:8558/docs/cli/reports-output — shows 4 sections
- http://localhost:8558/docs/cli/ci-cd — shows 3 sections

- [ ] **Step 3: Commit**

```bash
git add packages/website/src/pages/docs/cli/
git commit -m "feat(docs): add CLI track page files"
```

---

## Task 3: Create App Track Page Files

**Files:**
- Create: `packages/website/src/pages/docs/app/getting-started.astro`
- Create: `packages/website/src/pages/docs/app/running-evaluations.astro`
- Create: `packages/website/src/pages/docs/app/reading-results.astro`
- Create: `packages/website/src/pages/docs/app/ai-assistants.astro`
- Create: `packages/website/src/pages/docs/app/tool-analysis.astro`
- Create: `packages/website/src/pages/docs/app/library.astro`

- [ ] **Step 1: Create all six App page files**

Same pattern as CLI pages. Track label is "App". Slugs are: `app-getting-started`, `app-running-evaluations`, `app-reading-results`, `app-ai-assistants`, `app-tool-analysis`, `app-library`.

`packages/website/src/pages/docs/app/getting-started.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('app-getting-started')!;
---

<DocsLayout title={page.seoTitle} description={page.description} currentPath={page.href}>
  <p class="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">App</p>
  <h1 class="mt-3 font-display text-4xl font-bold tracking-tight">{page.label}</h1>
  <p class="mt-4 text-lg text-muted-foreground">{page.description}</p>
  <DocContent sections={page.sections} />
</DocsLayout>
...
`packages/website/src/pages/docs/app/running-evaluations.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('app-running-evaluations')!;
---
...
`packages/website/src/pages/docs/app/reading-results.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('app-reading-results')!;
---
...
`packages/website/src/pages/docs/app/ai-assistants.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('app-ai-assistants')!;
---
...
`packages/website/src/pages/docs/app/tool-analysis.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('app-tool-analysis')!;
---
...
`packages/website/src/pages/docs/app/library.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('app-library')!;
---
...
- [ ] **Step 2: Verify all six App pages load**

Check:
- http://localhost:8558/docs/app/getting-started
- http://localhost:8558/docs/app/running-evaluations
- http://localhost:8558/docs/app/reading-results
- http://localhost:8558/docs/app/ai-assistants
- http://localhost:8558/docs/app/tool-analysis
- http://localhost:8558/docs/app/library

- [ ] **Step 3: Commit**

```bash
git add packages/website/src/pages/docs/app/
git commit -m "feat(docs): add App track page files"
```

---

## Task 4: Create Reference Page Files

**Files:**
- Create: `packages/website/src/pages/docs/reference/configuration.astro`
- Create: `packages/website/src/pages/docs/reference/environment-variables.astro`

- [ ] **Step 1: Create both Reference page files**

`packages/website/src/pages/docs/reference/configuration.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('reference-configuration')!;
---

<DocsLayout title={page.seoTitle} description={page.description} currentPath={page.href}>
  <p class="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">Reference</p>
  <h1 class="mt-3 font-display text-4xl font-bold tracking-tight">{page.label}</h1>
  <p class="mt-4 text-lg text-muted-foreground">{page.description}</p>
  <DocContent sections={page.sections} />
</DocsLayout>
...
`packages/website/src/pages/docs/reference/environment-variables.astro`:
```astro
---
import DocsLayout from '@/layouts/DocsLayout.astro';
import DocContent from '@/components/docs/DocContent.astro';
...
const page = getDocPageBySlug('reference-env-vars')!;
---
...
- [ ] **Step 2: Verify both Reference pages load**

Check:
- http://localhost:8558/docs/reference/configuration
- http://localhost:8558/docs/reference/environment-variables

- [ ] **Step 3: Commit**

```bash
git add packages/website/src/pages/docs/reference/
git commit -m "feat(docs): add Reference track page files"
```

---

## Task 5: Update Existing Pages, Add Redirects, Clean Up Old Pages

**Files:**
- Modify: `packages/website/src/pages/docs/index.astro`
- Modify: `packages/website/src/pages/docs/installation.astro`
- Modify: `packages/website/src/pages/docs/quick-start.astro`
- Modify: `packages/website/astro.config.mjs`
- Delete: `packages/website/src/pages/docs/configuration.astro`
- Delete: `packages/website/src/pages/docs/usage.astro`
- Delete: `packages/website/src/pages/docs/app-mode.astro`

**Context:** The Overview page (`index.astro`) renders a grid of all non-overview pages pulled from `docsPages`. With the new structure it should show all 14 non-overview pages. The installation and quick-start pages just need their track label updated. Old pages are replaced by Astro redirects.

- [ ] **Step 1: Update `index.astro` description text**

Replace the current description paragraph in `packages/website/src/pages/docs/index.astro`:
...
<DocsLayout title='Documentation' description='Browse the MCPLab documentation.' currentPath='/docs'>
  <h1 class="font-display text-4xl font-bold tracking-tight">Documentation</h1>
  ...
  <div class="mt-10 grid gap-4 sm:grid-cols-2">
    {docsPages.filter((page) => page.slug !== 'overview').map((page) => (
      <a href={page.href} class="rounded-xl border border-border bg-card p-5 transition-colors hover:bg-accent/40">
        <div class="text-xs font-semibold uppercase tracking-[0.15em] text-muted-foreground mb-1">{page.track.replace('-', ' ')}</div>
        <div class="text-sm font-semibold text-primary">{page.label}</div>
        <div class="mt-2 text-sm text-muted-foreground">{page.description}</div>
      </a>
    ))}
  </div>
</DocsLayout>
...
- [ ] **Step 2: Update section label in `installation.astro` and `quick-start.astro`**

In `packages/website/src/pages/docs/installation.astro`, change:
```astro
  <p class="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">Getting Started</p>
```
(already correct — verify it says "Getting Started" and leave unchanged)

In `packages/website/src/pages/docs/quick-start.astro`, verify same. Update slugs if they changed. These pages are unchanged and can be left as-is.
...
- [ ] **Step 4: Delete the three old page files**

```bash
rm packages/website/src/pages/docs/configuration.astro
rm packages/website/src/pages/docs/usage.astro
rm packages/website/src/pages/docs/app-mode.astro
```

- [ ] **Step 5: Verify redirects work and sidebar is correct**

Start dev server and check:
- http://localhost:8558/docs — grid shows all 14 non-overview pages with track labels
- http://localhost:8558/docs/configuration — redirects to /docs/cli/configuration
- http://localhost:8558/docs/usage — redirects to /docs/cli/running-evaluations
- http://localhost:8558/docs/app-mode — redirects to /docs/app/getting-started
- Sidebar shows 4 sections: Getting Started (3), CLI (4), App (6), Reference (2)
- Prev/Next navigation links work correctly across all pages

- [ ] **Step 6: Commit**

```bash
git add packages/website/src/pages/docs/index.astro
git add packages/website/astro.config.mjs
git rm packages/website/src/pages/docs/configuration.astro
git rm packages/website/src/pages/docs/usage.astro
git rm packages/website/src/pages/docs/app-mode.astro
git commit -m "feat(docs): update overview, add redirects, remove old pages"
```
...
- [ ] Sidebar shows 4 sections with correct page counts (3 / 4 / 6 / 2)
- [ ] Prev/Next navigation flows correctly through all pages
- [ ] Search (`⌘K`) finds pages from all 4 sections
- [ ] All 3 redirects work
- [ ] `mcplab watch` does not appear anywhere in the docs
- [ ] `npx tsc --noEmit` passes in `packages/website`
